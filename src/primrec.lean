...
inductive primrec : (ptree → ptree) → Prop
| nil : primrec (λ _, nil)
| left : primrec (λ x, x.left)
| right : primrec (λ x, x.right)
| pair {f g} : primrec f → primrec g → primrec (λ x, (f x).node (g x))
| comp {f g} : primrec f → primrec g → primrec (f ∘ g)
| prec {f g} : primrec f → primrec g → primrec (λ x, g^[(f x).length] x) 

namespace primrec

theorem const : ∀ (x : ptree), primrec (λ _, x)
| ptree.nil := nil
| (ptree.node a b) := pair (const a) (const b)
